...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[admin/settings/branches] GET error:', e)
    const message = e instanceof Error ? e.message : 'Failed to load branches'
    return NextResponse.json(
      { error: 'Failed to load branches', detail: process.env.NODE_ENV === 'development' ? message : undefined },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existing) {
      return NextResponse.json({ error: 'A branch with this name already exists in this state' }, { status: 400 })
    }
    ...
    console.error('[admin/settings/branches] POST error:', e)
    const message = e instanceof Error ? e.message : 'Failed to create branch'
    return NextResponse.json(
      { error: 'Failed to create branch', detail: process.env.NODE_ENV === 'development' ? message : undefined },
      { status: 500 }
    )
  }
}
